...
Back in 2013 Torsten Seemann wrote up a list of [Minimum standards for bioinformatics command line tools](http://thegenomefactory.blogspot.com.au/2013/08/minimum-standards-for-bioinformatics.html) which provides ten useful tips to make Bioinformatics command line code easier to use. The list was later published in [GigaScience](http://www.gigasciencejournal.com/content/2/1/15]). The following article contains a few additions to Torsten's list which are inspired by my own experiences with installing, using and writing scientific software while working at [VLSCI](www.vlsci.org.au). 

The focus is on Unix command line tools, but some of the points apply more generally.

I doubt I've ever managed to follow all these principles at once in a single piece of software, but I see it a something to aspire to where appropriate.

## Use a standard license 

If you are going to make your code open source then I recommend using a standard open source license. The [OSI](http://opensource.org/) has a [good description](http://opensource.org/licenses) of the most common ones used. The best I can say for non-open source licenses is try to keep it simple: remember that humans have to read these things. 

A standard license provides minimum fuss for users and increases the chances that your software will be widely used. It is very common for research centers to install software on behalf of their users. Unsurprisingly such research centers (and their parent institutions) tend to be risk adverse when it comes to legal matters. A non-standard license is very likely to require vetting by lawyers, which can be a [protracted exercise](http://dilbert.com/strip/2013-08-11). Difficult licenses may be rejected or potential users may simply give up in frustration.

Make your license easy to find. The license should be clearly stated in the user documentation, preferably on the front page, near the start. Use the common name of the license in your documentation. For example, if you have chosen the [BSD 3-Clause License](http://opensource.org/licenses/BSD-3-Clause) then call it by that name. The user should not have to read the license to find out what it is. Obviously you will also need to make terms of the license available for the user to read. If your code is open source then I recommend putting it in a file called `LICENSE.txt` (or just `LICENSE`) in the top directory of the source repository. 

If your software includes other packages in its distribution then make sure the licenses for those packages are clearly indicated both in the distribution files and in the user documentation.

It may be tempting to take a standard license and add your own little twist to it, such as a "cite my work" clause. Don't do it! Even small adjustments will force research centers to send your license to the lawyers for scrutiny. If you want people to cite your work then I recommend putting that as a polite request in the user documentation rather than a clause in the license. Responsible scientists will do the right thing, especially if you make it easy for them. 
...
A log file is a godsend to anyone who is trying to find out what a program has done or why it might have gone wrong. I think the best way to do this is by providing a '`--log <filename>`' command-line argument. You may also consider allowing the log messages to optionally appear on the standard error device (`stderr`), though I think a file is generally better because it won't get lost by some accidental output redirection, and you won't clutter `stderr` with junk. You may also want to consider allowing the user to turn logging off - but I would recommend leaving it on by default. 

Log files are also incredibly useful for bug reporting, but you should warn users about sending sensitive information in bug reports. For example a log file might contain the names of files used, which might reveal patient identifiers or top-secret gene names that would give the game away. 
...
Research centers will often want to install your code from source, and they will generally want to install it for all users in a location of their own choosing. Ideally your program's behaviour should not be context dependent:
...
Shared mutable state is especially problematic in a concurrent setting so try to avoid it. A classic problem occurs with programs which write to files in the `/tmp` directory but do not protect against multiple instances clobbering each other's data. If you need to create temporary files then use a library designed for this purpose, such as the standard [tempfile](https://docs.python.org/3/library/tempfile.html) library from Python. Provide a command line argument that let's the user decide where temporary files should be stored. Some High Performance Computing (HPC) systems have special "scratch" filesystems for this purpose which can have non-standard paths.

If you really do need shared mutable state (multiple instances of your program reading and writing the same data) then consider using a database. [SQLite](http://www.sqlite.org/) is very simple to setup and use because it does not need a database server. [PostgreSQL](http://www.postgresql.org/) has all the bells and whistles you can imagine, but may be overkill for a lot of applications.
